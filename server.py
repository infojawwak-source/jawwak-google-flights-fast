...
from flights.search import SearchFlights

PORT = int(os.environ.get("PORT", "10000"))
API_KEY = os.environ.get("JAWAKK_GOOGLE_FLIGHTS_API_KEY", "").strip()
MAX_RESULTS = int(os.environ.get("MAX_RESULTS", "50"))

AIRPORTS = {a.name: a for a in Airport}
...
def airport_enum(code):
    ...
    try:
        ...
        # Some releases expose enum values rather than names.
        for a in Airport:
...
def cabin_enum(cabin):
    ...
    if c in ("premium_economy", "premium-economy", "premium"):
        return getattr(SeatType, "PREMIUM_ECONOMY", SeatType.ECONOMY)
    if c == "business":
...
def build_filters(q):
    from_code = str(q.get("from", "")).upper()
    to_code = str(q.get("to", "")).upper()
    depart = str(q.get("departDate", ""))
    ...
    legs = [
        FlightSegment(
            departure_airport=[[airport_enum(from_code), 0]],
            arrival_airport=[[airport_enum(to_code), 0]],
            travel_date=depart,
        )
    ]
    ...
    if ret:
        legs.append(
            FlightSegment(
                departure_airport=[[airport_enum(to_code), 0]],
                arrival_airport=[[airport_enum(from_code), 0]],
                travel_date=ret,
            )
        )
    ...
    return FlightSearchFilters(
        ...
        seat_type=cabin_enum(q.get("cabin", "economy")),
        stops=MaxStops.ANY,
...
def search_google(q):
    ...
    # The fli library performs a genuine Google Flights round-trip search
    # through Google's internal RPC and parses the itinerary as one result.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Return the library's complete typed itinerary objects.
            # This is intentionally raw/diagnostic for this first test so
            # we can verify that Google's round-trip price belongs to both legs.
            flights = [jsonable(x) for x in results]
            ...
            self._send(200, {
                ...
                "engine": "fli",
                "tripType": "round-trip" if q.get("returnDate") else "one-way",
                "pricePolicy": (
                    "Price is taken from the same fli Google Flights itinerary; "
                    "no separate return search or manual price summation is used."
                ),
                "flights": flights,
            })
...
if __name__ == "__main__":
    print(f"Jawwak Google Flights fli server listening on port {PORT}")
